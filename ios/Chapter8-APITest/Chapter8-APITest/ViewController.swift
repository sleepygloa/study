...
//  Chapter8-APITest
//
//  Created by seonho on 2022/05/27.
//
...
class ViewController: UIViewController {
    ...
    @IBAction func post(_ sender: Any) {
        ...
        //5. URLSession 객체를 통해 전송 및 응답할 처리 로직 작성
        let task = URLSession.shared.dataTask(with: request) { (data, response, error ) in
            //5-1. 서버가 응답이 없거나 통신이 실패 했을때
            if let e = error {
            ...
            DispatchQueue.main.async {
                do{
                    ...
                    print("An error has ocurred while parsing JSONObject : \(e.localizedDescription)")
                }
            }
        }
    ...
    @IBAction func json(_ sender: Any) {
        ...
        //3. URLRequest 객체 저으이 및 요청 내용 담기
        var request = URLRequest(url: url!)
        request.httpMethod = "POST"
        request.httpBody = paramData
