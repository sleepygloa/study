...
class RevealViewController:UIViewController{
    ...
    func setupView(){
        ...
        if let vc = self.storyboard?.instantiateViewController(withIdentifier: "sw_front") as? UINavigationController{
            ...
            //2. 읽어온 컨트롤러 클래스 전체에서 참조할 수 있도록 contentVC  속성에 저장한다
            self.contentVC = sideVC
            //3. 프론트 컨트롤러 객체를 메인 컨트롤러의 자식으로 등록한다
            self.addChild(vc) //프론트 컨트롤러를 메인 컨트롤러의 자식 뷰 컨트롤러로등록한다
    ...
    func getSideView(){
        ...
        //2. 다른 메소드에서도 참조 할 수 있도록 sideVC 속성에 저장한다
        self.sideVC = vc
        
        //3. 읽어온 사이드 바 컨트롤러 객체를 컨테이너 뷰 컨트롤러에 연결한아
        self.addChild(vc)
        self.view.addSubview(vc.view)
        
        //4. 프론트 컨트롤러에 부모 뷰 컨트롤러가 바뀌었음을 알려준다
        vc.didMove(toParent: self)
        
        //5. 프론트 컨트롤러의 뷰를 제일 위로 올린다
